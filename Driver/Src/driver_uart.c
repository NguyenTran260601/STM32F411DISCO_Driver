 ...
 * @Note              -  none
 */
void USART_PeriClockControl(USART_Reg_t *pUSARTx, uint8_t En_or_Di)
{
	if(En_or_Di == ENABLE )
		{
			if(pUSARTx == USART1)
			{
				USART1_PCLOCK_ENABLE();
			}else if(pUSARTx == USART2)
			{
				USART2_PCLOCK_ENABLE();
			}else if(pUSARTx == USART6)
			{
				USART6_PCLOCK_ENABLE();
			}
		}else
		{
			if(pUSARTx == USART1)
			{
				USART1_PCLOCK_DISABLE();
			}else if(pUSARTx == USART2)
			{
				USART2_PCLOCK_DISABLE();
			}else if(pUSARTx == USART6)
			{
				USART6_PCLOCK_DISABLE();
			}
		}
}
 ...
 * @brief             -
 *
 * @param[in]         - base address of the UART handle
 * @param[in]         -
 * @param[in]         -
 *
 * @return            -  none
 ...
 * @fn      		  - USART_SendDataIT
 *
 * @brief             - This function sends data with interrupt
 * @param[in]         - base address of the UART peripheral
 * @param[in]         - address of Tx buffer
 * @param[in]         - data length
 *
 ...
 *
 * @brief             - This function receives data with interrupt
 * @param[in]         - base address of the UART peripheral
 * @param[in]         - address of Tx buffer
 * @param[in]         - data length
 *
 ...
 */
void USART_IRQITConfig(uint8_t IRQNumber, uint8_t En_or_Di)
{
	if(En_or_Di == ENABLE)
		{
			if(IRQNumber <= 31)
	...
	if(temp2 )
	{
		...
		if(temp1 & ( 1 << USART_SR_NF) )
		{
			/*
				...
				USART_DR register).
			*/
			USART_ApplicationEventCallback(pUSARTHandle,USART_SR_NF);
		}
...
uint8_t USART_GetFlagStatus(USART_Reg_t *pUSARTx, uint8_t FlagName)
{
...
void USART_ClearFlag(USART_Reg_t *pUSARTx, uint16_t FlagName)
{

}

void USART_PeripheralControl(USART_Reg_t *pUSARTx, uint8_t En_or_Di)
{
	if(En_or_Di == ENABLE)
	{
 ...
 */
__attribute((weak)) void USART_ApplicationEventCallback(USART_Handle_t *pUSARTHandle,uint8_t ApplicationEv)
{
